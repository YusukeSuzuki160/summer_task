...
impl Lexer {
    ...
    fn get_lexerme(&self) -> String {
        self.input[self.pos_start..self.last_pos]
            .iter()
            .collect::<String>()
            .clone()
    }
    ...
    fn q_fu(&mut self) -> Result<(), LexerError> {
        match self.readc() {
            'n' => {
    ...
    fn q_unit(&mut self) -> Result<(), LexerError> {
        match self.readc() {
            ...
            _ => Err(LexerError::InvalidToken(self.pos_current)),
        }
    }
    ...
    fn q_v(&mut self) -> Result<(), LexerError> {
        match self.readc() {
            'a' => {
                self.save(Tokens::ID);
                self.q_va()
    ...
    fn q_va(&mut self) -> Result<(), LexerError> {
        match self.readc() {
            'r' => {
                self.save(Tokens::VAR);
                self.q_sym()
            }
            c => {
                if c.is_alphabetic() || c.is_numeric() {
...
mod test {
    ...
    fn lexer_test() {
        ...
        input.push("if x = 2 then var y := 3 else var y := x * 4");
        input.push("fun f(x, y, z) = x + y * z");
        input.push("var x := 1; var y := 2;");
        input.push("var x := \"test\"");
        let mut expected = Vec::new();
        expected.push(vec![
            (Tokens::LET, "let".to_string()),
            (Tokens::ID, "x".to_string()),
            (Tokens::EQ, "=".to_string()),
            (Tokens::INT, "1".to_string()),
            (Tokens::IN, "in".to_string()),
            (Tokens::ID, "x".to_string()),
        ]);
        expected.push(vec![
            (Tokens::LET, "let".to_string()),
        ...
        expected.push(vec![
            ...
            (Tokens::ID, "x".to_string()),
            (Tokens::EQ, "=".to_string()),
            (Tokens::INT, "2".to_string()),
            (Tokens::THEN, "then".to_string()),
            (Tokens::VAR, "var".to_string()),
            (Tokens::ID, "y".to_string()),
            (Tokens::UNIT, ":=".to_string()),
            (Tokens::INT, "3".to_string()),
            (Tokens::ELSE, "else".to_string()),
            (Tokens::VAR, "var".to_string()),
            (Tokens::ID, "y".to_string()),
            (Tokens::UNIT, ":=".to_string()),
            (Tokens::ID, "x".to_string()),
            (Tokens::MULT, "*".to_string()),
            (Tokens::INT, "4".to_string()),
        ]);
        expected.push(vec![
            (Tokens::FUNC, "fun".to_string()),
            (Tokens::ID, "f".to_string()),
            (Tokens::LPAREN, "(".to_string()),
            (Tokens::ID, "x".to_string()),
            (Tokens::COMMA, ",".to_string()),
            (Tokens::ID, "y".to_string()),
            (Tokens::COMMA, ",".to_string()),
            (Tokens::ID, "z".to_string()),
